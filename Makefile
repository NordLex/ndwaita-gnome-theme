...
ifeq ($(DESTDIR),)
    ...
    ifeq ($(IS_UBUNTU), true)
		...
		mv -n $(PREFIX)/share/gnome-shell/gnome-shell-theme.gresource $(PREFIX)/share/gnome-shell/gnome-shell-theme.gresource.old || true
...
dist: _get_version
	count=1; \
	for color_variant in $(COLOR_VARIANTS) Oslo; \
	do \
		count_pretty=$$(echo "0$${count}" | tail -c 3); \
		(cd themes && tar -c "NdWaita-$${color_variant}"*) | \
			xz -z - > "$${count_pretty}-NdWaita-$${color_variant}_$(VERSION).tar.xz"; \
		count=$$((count+1)); \
	done; \
...
launchpad_release: _get_version
	...
	debuild -S -d; \
	dput ppa:NordLex/NdWaita /tmp/$(PKGNAME)/$(PKGNAME)_$(VERSION)_source.changes
